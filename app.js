...
*/
let evolve
const myPraty = game.party
for (let i = 0; i < myPraty.length; i++) {
  if (myPraty[i].starter) {
    for (let j = 0; j < pokemon.length; j++) {
      if (myPraty[i].number === pokemon[j].number) {
        evolve = pokemon[j + 1]
      }
    }
    myPraty.splice(i, 1, evolve)
  }
}
...
for (let i = 0; i < pokemon.length; i++) {
  if (pokemon[i].starter) {
    starterPok.push(pokemon[i].name)
  }
}
...
game.catchPokemon = function (pokemonObj) {
  game.party.push(pokemonObj)
  const pokeballQun = game.items.find((items) => items.name === 'pokeball')
  if (pokeballQun) {
    pokeballQun.quantity--
  }
}
